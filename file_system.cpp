...
int change_dir(char dir_name[]){
    ...
    if(current_dir_table->dirs[unit_index].type == 1){
        printf("Not A Directory, Please Check Again!\n");
        return -1;
    }
...
int add_dir_unit(dir_table* system_dir_table, char file_name[], int type, int fcb_block_number){
    ...
    if(find_unit_in_table(system_dir_table, file_name)!=-1){
...
int create_dir(char dir_name[]){
    ...
    if(add_dir_unit(current_dir_table, dir_name, 0, dir_block) == -1){
        printf("Problem in add dir unit\n");
        return -1;
    }
//    printf("This is the check point of the creat_dir function\n");
...
int write_exec(fcb* system_fcb, char content[]){
    ...
    if(system_fcb->data_size == file_size){
        printf("This File Has Reached Its File Size, You Can Try to "
               "Create A New File to Store It!\n");
    }
    return 0;
}
...
int read_all(char file_name[]){
    ...
    int file_size = system_fcb->file_size;

    read_exec(system_fcb, file_size * 1024);
